...
if (!function_exists('fs_add_staff')) {
	function fs_add_staff()
	{
		...
		function wpt_save_staff_meta($post_id, $post)
		{
			...
			$staff_meta['_email'] = isset($_POST['_email']) ?? $_POST['_email'];
			$staff_meta['_phone'] = isset($_POST['_phone']) ?? $_POST['_phone'];
			$staff_meta['_facebook'] = isset($_POST['_facebook']) ?? $_POST['_facebook'];
			$staff_meta['_twitter'] = isset($_POST['_twitter']) ?? $_POST['_twitter'];
...
if (!function_exists('fs_add_sermons')) {
	function fs_add_sermons()
	{
		...
		function wpt_save_sermon_meta($post_id, $post)
		{
			...
			// OK, we're authenticated: we need to find and save the data
			// We'll put it into an array to make it easier to loop though.
			$sermon_meta['_video']  = isset($_POST['_video']) ?? $_POST['_video'];
			$sermon_meta['_youtube']  = isset($_POST['_youtube']) ?? $_POST['_youtube'];
			$sermon_meta['_audio']  = isset($_POST['_audio']) ?? $_POST['_audio'];
			$sermon_meta['_notes']  = isset($_POST['_notes']) ?? $_POST['_notes'];
			// Add values of $events_meta as custom fields
